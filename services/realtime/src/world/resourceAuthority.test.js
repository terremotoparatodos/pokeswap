...
test('A and B race for the same tree: exactly one reservation, a clean refusal for the other', async () => {
  ...
  const [first, second] = await Promise.all([
    authority.requestWork(actors.get('a'), intent(1, 25)),
    authority.requestWork(actors.get('b'), intent(1, 6)),
  ])
...
test('B asking for a tree A is already working is refused', async () => {
  ...
  await authority.requestWork(actors.get('a'), intent(1, 25))
  const refused = await authority.requestWork(actors.get('b'), intent(1, 6))
...
test('completion depletes once, settles once, and respawns at the world time', async () => {
  ...
  assert.equal((await authority.requestWork(actors.get('b'), intent(2, 6))).reason, 'depleted')
...
test('a far node and a Pokémon the player does not own are refused', async () => {
  ...
  actors.get('a').tx += 5
  assert.equal((await authority.requestWork(actors.get('a'), intent(1, 25))).reason, 'too-far')
  actors.get('a').tx -= 5
  assert.equal((await authority.requestWork(actors.get('a'), intent(2, 6))).reason, 'not-owner')
  ...
  assert.equal((await authority.requestWork(actors.get('a'), intent(3, 25))).reason, 'wrong-area')
})

test('a repeated request id is answered without a second reservation', async () => {
  const { authority, actors, nodes } = setup()
  const [first, again] = await Promise.all([
    authority.requestWork(actors.get('a'), intent(7, 25)),
    authority.requestWork(actors.get('a'), intent(7, 25)),
  ])
  assert.equal(first.ok, true)
...
test('one action per player and per Pokémon', async () => {
  ...
  assert.equal((await authority.requestWork(actors.get('a'), intent(1, 25))).ok, true)
  actors.set('a2', { id: 'a', areaId: 'pradera', ...stands[0] })
  assert.equal((await authority.requestWork(actors.get('a2'), intent(2, 26, other.id))).reason, 'actor-busy')
  actors.set('b', { id: 'b', areaId: 'pradera', ...stands[0] })
  assert.equal((await authority.requestWork(actors.get('b'), intent(1, 25, other.id))).reason, 'pokemon-busy')
})
...
test('SKILLS refusing an attempt leaves no trace on the node', async () => {
  ...
  assert.equal((await authority.requestWork(actors.get('a'), intent(1, 25))).reason, 'level')
  assert.equal(nodes.length, 0)
  assert.equal((await authority.requestWork(actors.get('b'), intent(1, 6))).reason, 'level', 'the claim was released')
})
...
test('a request with someone else’s Pokémon, still in flight, never makes the node busy for a legitimate player', async () => {
  ...
  const hostile = authority.requestWork(actors.get('b'), intent(1, 25)) // B does not own 25
  const legit = await authority.requestWork(actors.get('a'), intent(1, 25))
...
test('an attempt SKILLS is about to refuse never makes the node busy either', async () => {
  ...
  const { authority, actors } = setup({ skills })
  const refused = authority.requestWork(actors.get('b'), intent(1, 6))
  assert.equal((await authority.requestWork(actors.get('a'), intent(1, 25))).ok, true)
  open()
...
test('one attempt in flight per player: a concurrent second one costs no ownership read', async () => {
  ...
  const [first, second] = await Promise.all([
    authority.requestWork(actors.get('a'), intent(1, 25)),
    authority.requestWork(actors.get('a'), intent(2, 25)),
  ])
  assert.equal(first.ok, true)
