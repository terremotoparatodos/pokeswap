// GENERATED by scripts/integration/bundle-skills.mjs from
...
// src/features/skills/domain/balance.ts
var MAX_SKILL_LEVEL = 50;
var XP_CURVE = { linear: 20, base: 15, growth: 1.19 };
...
var CROPS = [
  {
    ...
    growMs: 1.5 * MIN,
    xp: { plant: 8, tend: 4, harvest: 25 },
    harvest: { itemId: "oran_berry", min: 2, max: 3 }
  },
  {
    id: "medicinal",
    ...
    plotKinds: ["town", "fertile"],
    growMs: 3 * MIN,
    xp: { plant: 25, tend: 12, harvest: 100 },
    harvest: { itemId: "medicinal_herb", min: 2, max: 3 }
  },
  {
    id: "leppa",
    name: "Baya Zanama",
    ...
    plotKinds: ["town", "fertile"],
    growMs: 5 * MIN,
    xp: { plant: 60, tend: 30, harvest: 240 },
    harvest: { itemId: "leppa_berry", min: 2, max: 4 }
  },
  {
    id: "sitrus",
    name: "Baya Zidra",
    ...
    plotKinds: ["fertile"],
    growMs: 8 * MIN,
    xp: { plant: 140, tend: 70, harvest: 550 },
    harvest: { itemId: "sitrus_berry", min: 2, max: 4 }
  },
  {
    id: "revival",
    name: "Hierba Revivir",
    ...
    plotKinds: ["fertile"],
    growMs: 12 * MIN,
    xp: { plant: 280, tend: 130, harvest: 1120 },
    harvest: { itemId: "revival_herb", min: 1, max: 2 }
  }
];
var CROP_BY_ID = new Map(CROPS.map((entry) => [entry.id, entry]));
function checkPlotTransition(action, plot, cropId) {
  switch (action) {
    case "plant": {
      if (plot.stage !== "EMPTY") return "plot_not_empty";
      const crop = cropId ? CROP_BY_ID.get(cropId) : void 0;
...
// src/features/skills/domain/skills.ts
var SKILLS = {
  woodcutting: {
    id: "woodcutting",
    name: "Talar",
    ...
    working: "Talando",
    icon: "\u{1F332}",
    pitch: "Tu Pok\xE9mon tala \xE1rboles y te trae madera."
  },
  mining: {
    id: "mining",
    name: "Miner\xEDa",
  ...
  farming: {
    ...
    working: "Cultivando",
    icon: "\u{1F331}",
    pitch: "Tu Pok\xE9mon siembra, cuida y cosecha bayas y hierbas."
  }
};

// src/features/skills/domain/messages.ts
function requirementLine(skillId, level) {
  return `Requiere ${SKILLS[skillId].name} ${level}`;
}
function refusalMessage(reason, skillId, requiredLevel, minAptitude) {
  switch (reason) {
    case "level_too_low":
      return skillId && requiredLevel ? requirementLine(skillId, requiredLevel) : "Todav\xEDa no sab\xE9s trabajar esto.";
    case "aptitude_too_low":
      return skillId && minAptitude ? `Este trabajo pide un Pok\xE9mon con aptitud ${minAptitude}+ en ${SKILLS[skillId].name}.` : "Este Pok\xE9mon no puede con este trabajo.";
    case "plot_not_empty":
      return "La parcela ya est\xE1 sembrada.";
    case "plot_not_growing":
      return "No hay nada creciendo para cuidar.";
    case "plot_not_ready":
      return "Todav\xEDa no est\xE1 lista para cosechar.";
    case "already_tended":
      return "Ya la cuidaste esta vez.";
    case "wrong_plot_kind":
      return "Esto s\xF3lo crece en tierra f\xE9rtil.";
    case "unknown_crop":
      return "No se puede plantar eso.";
    case "duplicate_action":
      return "Esa acci\xF3n ya se registr\xF3.";
...
function levelUpLine(skillId, before, after) {
  return after > before ? `${SKILLS[skillId].name} ${before} \u2192 ${after}` : null;
}

// src/features/skills/domain/materials.ts
var m = (id, name, skill, grade, use, purpose, planned = true) => ({ id, name, skill, grade, use, purpose, planned });
var MATERIALS = [
  m("common_log", "Tronco com\xFAn", "woodcutting", 1, "building", "Madera b\xE1sica: construcci\xF3n y venta."),
  ...
  m("boreal_log", "Madera boreal", "woodcutting", 4, "valuable", "La madera m\xE1s valiosa: construcciones avanzadas y venta alta."),
  m("stone", "Piedra", "mining", 1, "building", "Material b\xE1sico: construcci\xF3n y venta."),
  m("coal", "Carb\xF3n", "mining", 2, "fuel", "Combustible: sin \xE9l no se funde metal."),
  ...
  m("crystal", "Cristal", "mining", 5, "valuable", "Cristal puro de cueva: el recurso m\xE1s raro del arco 1\u201350."),
  m("oran_berry", "Baya Aranja", "farming", 1, "consumable", "Cura un poco a un Pok\xE9mon."),
  ...
  m("sitrus_berry", "Baya Zidra", "farming", 4, "consumable", "Cura mucho a un Pok\xE9mon."),
  m("revival_herb", "Hierba Revivir", "farming", 5, "valuable", "Revive a un Pok\xE9mon debilitado.")
];
var MATERIAL_BY_ID = new Map(MATERIALS.map((entry) => [entry.id, entry]));
var materialName = (id) => MATERIAL_BY_ID.get(id)?.name ?? id;

// src/features/skills/domain/resources.ts
var RESOURCES = [
  // ── Talar ────────────────────────────────────────────────────────────────
  {
    id: "common_tree",
    skill: "woodcutting",
    name: "\xC1rbol com\xFAn",
    tier: "muy b\xE1sico",
    requiredLevel: 1,
    minAptitude: 1,
    xp: 10,
...
// src/features/skills/domain/roadmap.ts
var WHERE = {
  common_tree: "\xC1rboles en todas partes, desde la salida de la ciudad",
  pine_tree: "Pinos del Bosque Umbr\xEDo",
  hardwood_tree: "Lo profundo del Bosque Umbr\xEDo",
  boreal_tree: "Tundra Helada, lejos de la entrada",
  stone_outcrop: "Rocas sueltas, desde la salida de la ciudad",
  coal_seam: "Rocas oscuras, un poco m\xE1s lejos",
  iron_vein: "Pe\xF1ascos del Desierto y la Tundra",
...
function buildUnlocks() {
  ...
  for (const resource of RESOURCES) {
    list.push({
      ...
      id: resource.id,
      title: resource.name,
      detail: `${materialName(resource.drop.itemId)} \xB7 ${WHERE[resource.id] ?? ""}`.trim()
    });
  }
  for (const crop of CROPS) {
    const where = crop.plotKinds.includes("town") ? "Huerta comunal" : "S\xF3lo tierra f\xE9rtil";
    list.push({ skillId: "farming", level: crop.requiredLevel, kind: "crop", id: crop.id, title: crop.name, detail: `Plantar \xB7 ${where}` });
  }
  for (const skillId of ["woodcutting", "mining", "farming"]) {
  ...
  const order = { resource: 0, crop: 0, rhythm: 1 };
...
function clampAptitude(value) {
  if (!Number.isFinite(value)) return 1;
  return Math.max(1, Math.min(5, Math.round(value)));
}

// src/features/skills/domain/aptitude/aptitudeRules.ts
var APTITUDE_BASE = 2;
var TYPE_TIERS = {
...
// src/features/skills/domain/aptitude/overrides.ts
var APTITUDE_OVERRIDES = [
  // ── Talar ────────────────────────────────────────────────────────────────
  { speciesId: 123, aptitudes: { woodcutting: 5 }, reason: "Scyther: guada\xF1as; tala por excelencia." },
  ...
  { speciesId: 141, aptitudes: { woodcutting: 4 }, reason: "Kabutops: brazos de guada\xF1a." },
  { speciesId: 399, aptitudes: { woodcutting: 4 }, reason: "Bidoof: roe troncos sin parar (Pok\xE9dex)." },
  { speciesId: 400, aptitudes: { woodcutting: 5 }, reason: "Bibarel: construye diques talando \xE1rboles (Pok\xE9dex)." },
  { speciesId: 185, aptitudes: { woodcutting: 1 }, reason: "Sudowoodo: se hace pasar por \xE1rbol; no tala a sus primos." },
  // ── Minería ──────────────────────────────────────────────────────────────
  ...
  { speciesId: 67, aptitudes: { mining: 5, woodcutting: 5 }, reason: "Machoke: el trabajador pesado cl\xE1sico (Pok\xE9dex)." },
  ...
  { speciesId: 81, aptitudes: { mining: 4 }, reason: "Magnemite: atrae el mineral met\xE1lico; fr\xE1gil para golpear." },
  { speciesId: 299, aptitudes: { mining: 4 }, reason: "Nosepass: br\xFAjula viviente, encuentra vetas; lento." },
  // ── Agricultura ──────────────────────────────────────────────────────────
  { speciesId: 241, aptitudes: { farming: 5 }, reason: "Miltank: el Pok\xE9mon de granja por excelencia." },
  { speciesId: 182, aptitudes: { farming: 5 }, reason: "Bellossom: su danza llama al sol." },
  { speciesId: 192, aptitudes: { farming: 5 }, reason: "Sunflora: convierte la luz en energ\xEDa." },
  { speciesId: 421, aptitudes: { farming: 5 }, reason: "Cherrim: florece con el sol." },
  { speciesId: 251, aptitudes: { farming: 5 }, reason: "Celebi: guardi\xE1n de los bosques." },
...
// src/features/skills/domain/aptitude/speciesFacts.generated.ts
var SPECIES_FACTS_ROWS = [
...
// src/features/skills/domain/aptitude/speciesFacts.ts
var BY_ID = new Map(
  SPECIES_FACTS_ROWS.map(([speciesId, name, types, baseStats]) => [speciesId, { speciesId, name, types, baseStats }])
);
var SPECIES_COUNT = BY_ID.size;
function speciesFacts(speciesId) {
  return BY_ID.get(speciesId) ?? null;
}

// src/features/skills/domain/aptitude/aptitude.ts
var FALLBACK_APTITUDE = APTITUDE_BASE;
function derivedAptitude(facts, skill) {
...
var DEFAULT_OVERRIDES = indexOverrides(APTITUDE_OVERRIDES);
function resolveAptitude(speciesId, skill, overrides = DEFAULT_OVERRIDES) {
  const patched = overrides.get(speciesId)?.aptitudes[skill];
  if (patched !== void 0) return { value: patched, source: "override" };
...
// src/features/skills/domain/xpCurve.ts
function xpToNextLevel(level, params = XP_CURVE) {
  return Math.round(params.linear * level + params.base * Math.pow(params.growth, level));
}
function buildThresholds(maxLevel = MAX_SKILL_LEVEL, params = XP_CURVE) {
  const table = [0, 0];
  for (let level = 1; level < maxLevel; level++) table.push(table[level] + xpToNextLevel(level, params));
  return table;
}
var THRESHOLDS = buildThresholds();
var sanitize = (xp) => Number.isFinite(xp) && xp > 0 ? Math.floor(xp) : 0;
...
function cappedGain(current, gain) {
...
function createSkillsService(ports) {
  ...
  function authorizeWorkAttempt(input) {
    const actionId = typeof input?.actionId === "string" ? input.actionId : "";
    const refuse = (reason, extra = {}) => ({
      allowed: false,
      actionId,
      reason,
      message: refusalMessage(reason, extra.skillId ?? null, extra.requiredLevel ?? null, extra.minAptitude ?? null),
      skillId: null,
      requiredLevel: null,
      playerLevel: null,
      minAptitude: null,
  ...
  function settleWork(actionId, context) {
    ...
    const settlement = {
      ...
      skillId,
      outcome: pays ? "completed" : "cancelled",
      xpGained,
      rewards,
...
// services/realtime/src/world/terrain.js
function hash2(x, y, seed) {
  let h = Math.imul(x | 0, 668265261) ^ Math.imul(y | 0, 374761393) ^ Math.imul(seed | 0, 2654435769);
...
function createSkillsWorldPolicy(options) {
  ...
  async function ensurePlayer(playerId) {
    ...
    xp.set(playerId, { ...EMPTY_XP, ...state.xp });
  }
  function prune() {
    const cutoff = now() - 15 * 6e4;
    for (const [id, work] of authorizations) if (work.authorizedAt < cutoff) {
      authorizations.delete(id);
      committed.delete(id);
    }
  }
  function targetOf(attempt) {
    if (attempt.farm) {
      const farm = attempt.farm;
      return {
        kind: "farm",
        action: farm.action,
        plot: { plotId: attempt.node.id, kind: farm.plotKind, stage: farm.stage, cropId: farm.cropId ?? null, tended: farm.tended },
        cropId: farm.action === "plant" ? farm.requestedCropId : farm.cropId
      };
    }
  ...
  return {
    ...
    async settleWork(settlement) {
      ...
      try {
        stored = await options.store.commitWork({
          ...
          rulesVersion: paid.rulesVersion,
    ...
    cancelWork(cancellation) {
      ...
      if (closed) {
        staged.delete(cancellation.actionId);
        committed.set(cancellation.actionId, closed);
      }
    },
    rulesVersion: SKILLS_RULES_VERSION
  };
