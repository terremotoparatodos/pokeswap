...
// on-screen pace over the sender's gait (p5–p95; 1 = exact).
...
function summarise(results) {
  ...
  for (const [key, { name, runs }] of groups) {
    out.set(key, {
      ...
      resyncs: runs.reduce((s, r) => s + (r.resyncs ?? 0), 0),
      pace: runs[0].visual.pace ? `${mean(runs.map(r => r.visual.pace.p5)).toFixed(2)}–${mean(runs.map(r => r.visual.pace.p95)).toFixed(2)}` : '—',
    })
  }
  return out
}
...
console.log('| scenario | snaps | max snap | stall ms/s | delay p50 | delay p95 | tiles unseen | pace p5–p95 |')
console.log('|---|---:|---:|---:|---:|---:|---:|---:|')
for (const [key, a] of after) {
  const b = before.get(key)
  if (!b) continue
  console.log(`| ${key} ${a.name} | ${cell(b.snaps, a.snaps)} | ${cell(b.maxSnap, a.maxSnap)} | ${cell(b.stall, a.stall)} | ${cell(b.p50, a.p50)} | ${cell(b.p95, a.p95)} | ${cell(b.unseen, a.unseen)} | ${cell(b.pace, a.pace)} |`)
}
