...
> **Base consolidada pre-R34 = R33 + Town 3D.** `integration/pre-r34-town-3d` toma el HEAD aprobado de R33 (`bd03e4b`) y le integra, por merge `--no-ff`, la ciudad 3D aprobada (`feat/town-3d-models @ 87504b7`): Ciudad Corazón pasa a **64×51** con modelos 3D reales de sus edificios, puertas de ruta, faroles, fuentes, bancos y vallas autotile. Los modelos son **sólo visuales**: colisión, puertas, tap y picking siguen siendo el sistema de footprints. El City Mapping Lab (`/dev/city-lab`) y los árboles de ciudad siguen **DEV-only** y fuera del bundle. R33 no regresiona. **R34 arranca desde esta rama.** Doc: [`PRE_R34_TOWN_3D_CONSOLIDATION.md`](PRE_R34_TOWN_3D_CONSOLIDATION.md), con el detalle técnico de la ciudad en [`TOWN_3D_INTEGRATION_HANDOFF.md`](TOWN_3D_INTEGRATION_HANDOFF.md).
...
areas/
  ...
  hearthomeTerrain.ts  Terreno 64x51 (s calle, g pasto, p plaza, t bosque) trazado del mapa
