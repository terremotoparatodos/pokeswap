// Logging scene overlay (R31-C3, SKILLS-1): everything the player sees of
// Talar inside the WildLands renderer — harvestable trees, proximity markers,
// the bite of each blow, leaves and splinters, the tree giving way, stumps and
// regrowth.
//
// The lifecycle, rings, markers, worker and pops shared with Minería live in
// `overworld/gatheringOverlayCore.ts`; this file keeps what is Talar's own —
// tree art, splinters and leaves, and the fall on the action that takes the
// last charge. The Pokémon does the chopping; the player holds no axe.
//
// Browser runtime only (it builds canvases). Every decision comes from pure
// modules: node state (its owner), tree visual state, the chopping timeline,
// leaves and rarity. It never writes anything outside local memory.
...
export type StartLogging = StartGathering<ChoppingTimeline>
...
export class LoggingOverlay extends GatheringOverlayCore<StartLogging, ActiveAction, TreeVisualView, VisibleNode<TreeVisualView>> {
  ...
  protected buildView({ target, status, state, adjacent, targeted, working, detected }: ViewInput): TreeVisualView {
    return treeVisual({
      ...
      rareTree: RARE_TREES.has(target.resource.id),
      detected,
    })
  }
    ...
    if (action?.target.nodeId === target.nodeId) {
      ...
      if (pose.fall > 0) {
        if (pose.phase === 'fell') {
          ...
          dx += away * pose.fall * 4
          dy += pose.fall * 3
          if (pose.fall > 0.7) art = loggingTreeArt(target.resource.id, decor.kind, 'stump')
        } else {
          ...
          art = loggingTreeArt(target.resource.id, decor.kind, 'stump')
        }
      }
    }
