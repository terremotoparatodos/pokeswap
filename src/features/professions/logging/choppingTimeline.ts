...
export function choppingTimeline(actionSeconds: number, felling: boolean): ChoppingTimeline {
  const chops = Math.max(MIN_CHOPS, Math.min(MAX_CHOPS, Math.round(actionSeconds / 6)))
