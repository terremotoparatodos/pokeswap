// Gathering overlay core (R31-Z): what mining, logging and foraging share
...
// linger, done, cancel), the selection rings, the proximity bubbles and
// prospecting glints, the tool held by the player, the worker and the reward
// pops.
//
// Each profession keeps what makes it look like itself: its art, its timeline
// and pose, its particles, how it restyles the host prop, and anything special
// (the falling tree, the herb patch scan). Fishing and alchemy do not extend
// this: casting and brewing are different lifecycles.
//
// Browser runtime only (sprites become canvases). Presentation only: results
// come from the demo session through `onResult`.
...
import { inspectDemoNode, type DemoNodeTarget, type DemoState, type NodeInspection } from '../demo/demoSession'
...
import { resolveNodeStatus, type NodeStatus } from '../ui/nodeStatus'
...
export interface StartGathering<Timeline extends GatheringTimeline = GatheringTimeline, Tier = number> {
  ...
  /** Applies the action (local demo) when the timeline reaches its result. */
...
export interface ViewInput {
  ...
  readonly inspection: NodeInspection
...
> implements SceneOverlay {
  ...
  targetAt(area: Area, tx: number, ty: number): DemoNodeTarget | null {
    ...
    if (world) {
      ...
      if (placement && node && this.ownsNode(node.id)) target = { nodeId: placement.nodeId, node, biome: placement.biome }
    }
  ...
  protected viewFor(target: DemoNodeTarget, tx: number, ty: number): View {
    ...
    const view = this.buildView({
      target, inspection, adjacent, targeted, working,
  ...
  /** Proximity bubbles and prospecting glints over the visible nodes. */
  ...
  /** The tool in the player's hand, offset to the side it is swung from. */
  protected toolSprite(player: OverlayPlayer, art: PixelArt, lift: number): OverlaySprite {
    const offsetX = player.dir === 'right' ? 5 : player.dir === 'left' ? -5 : player.dir === 'down' ? 4 : -4
    const offsetY = player.dir === 'down' ? 1 : player.dir === 'up' ? -1 : 0
    return { wx: player.x + offsetX, wy: player.y + offsetY, sprite: toSprite(art, false), lift, depthBias: player.dir === 'up' ? -0.6 : 0.6 }
  }
