<template>
  <section class="lab">
    <div class="lab-bar">
      ...
      <button
        v-for="landmark in LOGGING_LANDMARKS"
      ...
      <label class="lab-worker">
        ...
        <select :value="session.state.value.workers.woodcutting ?? ''" aria-label="Pokémon trabajador" @change="setWorker(($event.target as HTMLSelectElement).value)">
    ...
    <div class="lab-stage">
      <canvas
        ...
        class="lab-canvas"
        tabindex="0"
        aria-label="Bosque de Pradera Brisa en el motor real de WildLands. Tocá un árbol marcado para talarlo."
      ...
      />
      <div class="lab-top">
        <ProfessionHud :session="session" profession="woodcutting" />
      ...
      <div class="lab-dock">
        ...
        <div v-else class="lab-card">
          <LoggingActionCard
            ...
            @chop="controller.chop()"
            @close="controller.close()"
          />
        </div>
      </div>
    </div>
    <p class="lab-note">Solo una parte de los árboles aloja un nodo (densidad por ancla, determinista por semilla): el bosque sigue siendo bosque.</p>
  </section>
</template>
...
// R31-C3 field lab: the real WildLands engine in the Pradera woods, without
// presence or Supabase, with the logging overlay attached.
const props = defineProps<{ session: ProfessionDemoSession }>()

const LOGGING_LANDMARKS = PRADERA_LANDMARKS.filter(landmark => NODE_BY_ID.get(landmark.definitionId)?.profession === 'woodcutting')

const canvasRef = ref<HTMLCanvasElement | null>(null)
const game = shallowRef<WildlandsGame | null>(null)
const spawnId = ref('common_tree')
...
const controller = useLoggingController(props.session, () => game.value, () => (detection.value < 0 ? null : detection.value))
const highlight = computed(() => (controller.outcome.value?.ok ? controller.outcome.value.placements : []))
const setWorker = (value: string) => props.session.update(state => setDemoWorker(state, 'woodcutting', value ? Number(value) : null))

function createGame(): void {
  controller.detach()
  game.value?.destroy()
  if (!canvasRef.value) return
  const landmark = LOGGING_LANDMARKS.find(entry => entry.definitionId === spawnId.value) ?? LOGGING_LANDMARKS[0]
  const created = new WildlandsGame(canvasRef.value, {
    ...
    isWorldObject: hit => controller.isTree(hit),
  })
  game.value = created
  controller.close()
  controller.attach()
...
/** A free, dry tile two steps from the tree (so the approach is visible), facing it. */
