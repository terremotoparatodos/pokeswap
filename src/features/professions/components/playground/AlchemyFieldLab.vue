<template>
  <section class="lab">
    <div class="lab-bar">
      <span class="pf-kicker">Mesa de Alquimia</span>
      <span class="pf-chip">{{ station ? `en ${station.tx}, ${station.ty}` : 'sin claro libre' }}</span>
      <button type="button" class="pf-chip lab-go" @click="createGame">Volver a la mesa</button>
    ...
    <div class="lab-stage">
      <canvas
        ...
        class="lab-canvas"
        tabindex="0"
        aria-label="Claro de Pradera Brisa con la Mesa de Alquimia, en el motor real de WildLands. Tocá la mesa para abrirla."
      ...
      <div class="lab-dock">
        <div v-if="bagOpen" class="lab-bag">
          <InventoryGrid :session="session" compact />
        </div>
        <p v-if="!controller.open.value" class="lab-hint">Caminá hasta la mesa de alquimia y tocála para abrirla.</p>
        <div v-else class="lab-card">
          <AlchemyStationCard
            :recipes="controller.recipes.value"
            :detail="controller.view.value"
            :phase="controller.phase.value"
            :quantity="controller.quantity.value"
            :max-quantity="controller.maxQuantity.value"
            :level="controller.level.value"
            :outcome="controller.outcome.value"
            :progress="controller.progress.value"
            @select="controller.select($event)"
            @quantity="controller.setQuantity($event)"
    ...
    <p class="lab-note">
      La mesa no está guardada en ningún lado: se deriva del mundo (primer claro libre junto al spawn), igual que los nodos de R31-A.
      Preparar no consume energía ni herramienta; el costo son los ingredientes.
    </p>
  </section>
</template>
...
// R31-C4 field lab: the real WildLands engine in a Pradera clearing, with the
// alchemy bench attached to the scene overlay.
...
const controller = useAlchemyController(props.session, () => game.value, () => PRADERA_SPAWN)
...
function createGame(): void {
  ...
  const tile = station.value ?? PRADERA_SPAWN
  controller.setStation(tile)
  const created = new WildlandsGame(canvasRef.value, {
    ...
    isWorldObject: hit => controller.isStation(hit),
  })
  game.value = created
  controller.close()
  controller.attach()
...
/** A free, dry tile two steps from the bench, facing it, so the walk is visible. */
function spawnBeside(tile: StationTile): { tx: number; ty: number; dir: Dir } {
  const world = new World(PRADERA_SEED)
