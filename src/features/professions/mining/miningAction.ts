...
export function miningTimeline(actionSeconds: number): MiningTimeline {
  const swings = Math.max(MIN_SWINGS, Math.min(MAX_SWINGS, Math.round(actionSeconds / 6)))
