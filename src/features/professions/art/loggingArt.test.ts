...
describe('logging items', () => {
  ...
  it('separates axe tiers and conditions', () => {
    const ok = axeIconArt(1)
    expect(opaqueCount(ok)).toBeGreaterThan(10)
    expect(opaqueCount(axeIconArt(1, 'broken'))).toBeLessThan(opaqueCount(ok))
    expect(axeIconArt(3).pixels).not.toEqual(axeIconArt(1).pixels)
    expect(axeIconArt(2, 'retired').pixels).not.toEqual(axeIconArt(2).pixels)
  })

  it('mirrors the swing for the other facing', () => {
    const right = axeSwingArt(2, 2, false)
    const left = axeSwingArt(2, 2, true)
...
describe('logging asset registry', () => {
  it('lists unique ids and every asset builds', () => {
    const ids = LOGGING_ASSETS.map(asset => asset.id)
    expect(new Set(ids).size).toBe(ids.length)
    for (const asset of LOGGING_ASSETS) {
  ...
  it('covers trees, axes, icons, effects and markers', () => {
    for (const kind of ['tree', 'tool', 'icon', 'fx', 'marker'] as const) {
      expect(LOGGING_ASSETS.some(asset => asset.kind === kind), kind).toBe(true)
    }
  })
})
